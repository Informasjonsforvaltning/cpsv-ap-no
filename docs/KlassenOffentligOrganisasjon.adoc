...
| Merknad 2 / _Note 2_ | Norsk utvidelse: Som range bruker CPSV-AP v.3.1.1 et gitt kontrollert vokabular. Usage note i CPSV-AP v.3.1.1 sier på den annen side "... likely to be ..." og ikke "må være" det oppgitte vokabularet. For å kunne dekke behov for å kunne oppgi administrative enheter som ikke er på den oppgitte EU-listen, har vi valgt å bruke `dct:Location` som range som andre tilsvarende spesifikasjoner også bruker. 

__Norwegian extension: CPSV-AP v3.1.1 uses a specific controlled vocabulary as the range. The usage note says though "... likely to be ..." and not "shall/must be" that given controlled vocabulary. In order to be able to specify administrative units that are not in that given controlled vocabulary from EU, we have chosen to use `dct:Location` as the range which other similar specifications also use.__
