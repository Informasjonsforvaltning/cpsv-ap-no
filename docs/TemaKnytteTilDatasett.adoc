...
Som også illustrert i figuren, kan et dokumentasjonskrav (`cv:Evidence`) også bekreftes ved en dokumentreferanse (`cccev:DocumentReference`) som kan være et datasett (`dcat:Dataset`), f.eks. «Vandelsattest». Ut fra den engelske usage note til <<Dokumentasjon>>, kan dokumentasjon (_evidence_) være en hvilken som helst ressurs som trengs for å utføre en tjeneste, også et utfylt søknadsskjema.
