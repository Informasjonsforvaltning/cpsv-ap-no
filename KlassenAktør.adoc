...
|Merknad 1|For aktør som er en offentlig organisasjon, bruk <<OffentligOrganisasjon>> isteden.
|Merknad 2|Kravsnivå endret fra vaglfri til anbeflt.
|Merknad 3|Norsk utvidelse: range endret fra `dct:Agent` til `foaf:Agent` (EU har sagt at dette skal endres i neste release av CPSV-AP, så dette vil etter hvert ikke være en norsk utvidelse).
